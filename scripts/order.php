<?php 
	...
	$stmt = $mysqli->prepare("SELECT name, quantity, id FROM `portfolio` WHERE `username` = ?"); //Select username, name, quantity, id, order and orderprice from portfolio

	$stmt->bind_param("s", $username);

	$stmt->execute(); 

	$stmt->bind_result($name, $quantity, $oldId);
